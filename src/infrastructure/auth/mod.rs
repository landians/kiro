...
#[cfg(test)]
mod tests {
    use super::{ACCESS_TOKEN_TTL_HOURS, AuthService, AuthServiceBuilder, REFRESH_TOKEN_TTL_HOURS};
    ...
    fn generate_and_validate_access_token() {
        ...
        assert!(claims.exp - claims.iat <= ACCESS_TOKEN_TTL_HOURS * 60 * 60);
    }
    ...
    fn generate_and_validate_refresh_token() {
        ...
        assert_eq!(claims.iss, "kiro-api");
        assert!(claims.exp - claims.iat <= REFRESH_TOKEN_TTL_HOURS * 60 * 60);
    }
