...
- 禁止在实现代码中直接使用 `crate::...` 这种从根路径开始的方式去调用方法、函数、常量、变量或模块成员。
- 跨模块依赖默认通过文件顶部的 `use` 语句显式导入，再在正文中使用本地名称，避免在函数体或表达式中出现长链路的根路径调用。
- `crate::...` 允许保留在 `use` 声明、类型重导出或极少数为了避免命名冲突且确有必要的场景中；如果只是为了少写一个 `use`，则不允许。
- 判断标准是：当某个根路径调用可以通过补充 `use` 或使用更近的模块路径提升可读性时，必须改写，不要把 `crate::...` 直接留在业务逻辑和控制流中。
...
- HTTP 接口错误映射应优先定义在 `interfaces/` 层的包装错误类型中，并由该包装类型实现 `IntoResponse`，避免在 controller 中散落大量 `AppError::new(...)` 与状态码映射逻辑。
- application 和 domain 层不直接承担 HTTP 状态码、响应体 envelope、`IntoResponse` 等协议语义；这些转换统一留在接口层。
...
- `trace_id` 面向客户端时只通过响应头 `X-Trace-Id` 暴露，不在 JSON 成功体或错误体中重复返回。
...
- controller 不应内联大段 HTTP 错误码映射；当某个模块存在成组错误映射时，应抽出 `interfaces/error/*` 之类的包装错误类型集中维护。
